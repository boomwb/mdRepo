...
```
git remote add origin git@github.com:xxx/ttt.git
//origin是远程仓库的别名
...
//推送:把本地的main分支与远程仓库的main分支关联起来


git pull <远程仓库名><远程分支名>:<本地分支名>
//拉取:git pull 默认把远程仓库origin的main拉取到本地
git push会自动合并

git fetch
//只获取远程仓库的修改,不会合并到本地仓库中
git remote -v
//查看当前仓库对应的远程仓库的别名与地址
...
git branch 查看分支
git checkout  git switch <分支名> 切换分支

合并到主线:
当前在main: git merge dev  //把dev分支合并到main

git branch -d(-D强制) dev  删除已经合并的dev分支
